...
This data set is imported from the following URL. 
```{r data import}
...
global_totl <- global %>% group_by(Country_Region,Date) %>% summarise(cases = max(Cases),deaths = max(Deaths),population = sum(Population))%>%mutate(death_per_mill = deaths*1000000/population)%>%mutate(cases_per_mill = cases*1000000/population)%>% select(c(Country_Region,Date,cases,deaths,death_per_mill,cases_per_mill,population))%>% ungroup()
...
### Visualizing the data - Cases and Deaths by year
```{r data viz1}
global_totl %>%filter(cases>0)%>% ggplot(aes(x= Date,y=cases))+
    ...
    labs(title="Covid19 Around The World", y=NULL)
```

##### Inference:
By looking at the above visualization we can observe that cases and deaths leveled off after 2022. To take a closer look at the data, we need to look at new_cases and new_deaths.
